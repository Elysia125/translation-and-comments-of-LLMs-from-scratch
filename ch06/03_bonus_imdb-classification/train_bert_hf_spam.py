...
class SpamDataset(Dataset):
    ...
    def __init__(self, csv_file, tokenizer, max_length=None, pad_token_id=50256, no_padding=False):
        ...
        self.max_length = max_length if max_length is not None else self._longest_encoded_length(tokenizer)  # 设置最大长度

        # 预处理文本数据
        self.encoded_texts = [
            tokenizer.encode(text)[:self.max_length]  # 对每个文本进行编码
        ...
        if not no_padding:
            # 对序列进行填充
            self.encoded_texts = [
                et + [pad_token_id] * (self.max_length - len(et))  # 使用pad_token_id填充到最大长度
...
def create_dataset_csvs(new_file_path):
    ...
    # 创建平衡数据集
    n_spam = df[df["Label"] == "spam"].shape[0]  # 获取垃圾邮件数量
    ham_sampled = df[df["Label"] == "ham"].sample(n_spam, random_state=123)  # 采样正常邮件
    balanced_df = pd.concat([ham_sampled, df[df["Label"] == "spam"]])  # 合并数据
    balanced_df = balanced_df.sample(frac=1, random_state=123).reset_index(drop=True)  # 打乱数据
    balanced_df["Label"] = balanced_df["Label"].map({"ham": 0, "spam": 1})  # 转换标签
...
class SPAMDataset(Dataset):
    ...
    def __init__(self, csv_file, tokenizer, max_length=None, pad_token_id=50256, use_attention_mask=False):
        ...
        # 预处理文本并创建注意力掩码
        self.encoded_texts = [
            tokenizer.encode(text, truncation=True, max_length=self.max_length)  # 编码并截断文本
            for text in self.data["Text"]
        ]
        self.encoded_texts = [
        ...
        if self.use_attention_mask:
            self.attention_masks = [
                self._create_attention_mask(et)  # 创建注意力掩码
                for et in self.encoded_texts
            ]
        else:
